package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_645_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _656_, _657_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_656_ == true) and (nil ~= _657_)) then
      local old = _657_
      local _
      ...
      return on_values({"ok"})
    elseif ((_656_ == false) and (nil ~= _657_)) then
      local msg = _657_
      ...
      elseif (specials["macro-loaded"])[module_name] then
        ...
        local function _662_()
          local _661_ = msg:gsub("\n.*", "")
          return _661_
        end
        return on_error("Runtime", _662_())
      end
    else
      return nil
    end
  ...
  local function run_command(read, on_error, f)
    local _665_, _666_, _667_ = pcall(read)
    if ((_665_ == true) and (_666_ == true) and (nil ~= _667_)) then
      local val = _667_
      return f(val)
    elseif (_665_ == false) then
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _671_ = type(subtbl)
        if (_671_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_671_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _674_, names)
          else
          end
        else
        end
  ...
  commands.apropos = function(_env, read, on_values, on_error, _scope)
    ...
    return run_command(read, on_error, _679_)
  end
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      do
        ...
        if ("function" == type(tgt)) then
          local _683_ = (compiler.metadata):get(tgt, "fnl/docstring")
          if (nil ~= _683_) then
            local docstr = _683_
  ...
  local function resolve(identifier, _690_, scope)
    local _arg_691_ = _690_
    local ___replLocals___ = _arg_691_["___replLocals___"]
    local env = _arg_691_
    local e
    local function _692_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _692_})
    local _693_, _694_ = pcall(compiler["compile-string"], tostring(identifier), {scope = scope})
    if ((_693_ == true) and (nil ~= _694_)) then
      local code = _694_
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _696_(_241)
      local _697_
      do
        local _698_ = utils["sym?"](_241)
        if (nil ~= _698_) then
          local _699_ = resolve(_698_, env, scope)
          if (nil ~= _699_) then
            _697_ = debug.getinfo(_699_)
          else
            _697_ = _699_
          end
        else
      ...
      if ((_G.type(_697_) == "table") and ((_697_).what == "Lua") and (nil ~= (_697_).source) and (nil ~= (_697_).linedefined) and (nil ~= (_697_).short_src)) then
        local source = (_697_).source
        local line = (_697_).linedefined
        local src = (_697_).short_src
        local fnlsrc
        do
          local t_702_ = compiler.sourcemap
          if (nil ~= t_702_) then
            t_702_ = (t_702_)[source]
          else
          end
          if (nil ~= t_702_) then
            t_702_ = (t_702_)[line]
  ...
  commands.doc = function(env, read, on_values, on_error, scope)
    ...
    return run_command(read, on_error, _707_)
  end
  ...
  commands.compile = function(env, read, on_values, on_error, scope)
    ...
    return run_command(read, on_error, _710_)
  end
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _712_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _712_) then
          local cmd_name = _712_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _714_ = commands[command_name]
      if (nil ~= _714_) then
        local command = _714_
      ...
      do
        ...
        do
          ...
          for k, v in pairs(readline.set_options({})) do
            local _719_, _720_ = k, v
            if ((nil ~= _719_) and (nil ~= _720_)) then
              local k_12_auto = _719_
              local v_13_auto = _720_
  ...
  local function repl(_3foptions)
    ...
    local function _726_(parser_state)
      local c = byte_stream(parser_state)
      table.insert(chars, c)
      return c
    end
    read, reset = parser.parser(_726_)
    ...
    if opts.registerCompleter then
      local function _730_()
        local _728_ = env
        local _729_ = opts.scope
        local function _731_(...)
          return completer(_728_, _729_, ...)
        end
        return _731_
      end
      opts.registerCompleter(_730_())
    else
    end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if not_eof_3f then
          do
            local _735_, _736_ = nil, nil
            local function _738_()
              local _737_ = opts
              _737_["source"] = src_string
              return _737_
            end
            _735_, _736_ = pcall(compiler.compile, x, _738_())
            if ((_735_ == false) and (nil ~= _736_)) then
              local msg = _736_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_735_ == true) and (nil ~= _736_)) then
              local src = _736_
              local src0
              if save_locals_3f then
              ...
              local _740_, _741_ = pcall(specials["load-code"], src0, env)
              if ((_740_ == false) and (nil ~= _741_)) then
                local msg = _741_
                clear_stream()
                on_error("Lua Compile", msg, src0)
              elseif (true and (nil ~= _741_)) then
                local _ = _740_
                local chunk = _741_
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function wrap_env(env)
    ...
    local function _434_(_, key, value)
      if utils["string?"](key) then
    ...
    local function _436_()
      local function putenv(k, v)
        ...
        if utils["string?"](k) then
          _437_ = compiler["global-unmangling"](k)
        else
          _437_ = k
        end
        return _437_, v
    ...
    return setmetatable({}, {__index = _432_, __newindex = _434_, __pairs = _436_})
  end
  local function current_global_names(_3fenv)
    local mt
    do
      local _439_ = getmetatable(_3fenv)
      if ((_G.type(_439_) == "table") and (nil ~= (_439_).__pairs)) then
        local mtpairs = (_439_).__pairs
        local tbl_11_auto = {}
        for k, v in mtpairs(_3fenv) do
          local _440_, _441_ = k, v
          if ((nil ~= _440_) and (nil ~= _441_)) then
            local k_12_auto = _440_
            local v_13_auto = _441_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
        mt = tbl_11_auto
      elseif (_439_ == nil) then
        mt = (_3fenv or _G)
      else
        mt = nil
      end
  ...
  local function load_code(code, _3fenv, _3ffilename)
    ...
    local _444_, _445_ = rawget(_G, "setfenv"), rawget(_G, "loadstring")
    if ((nil ~= _444_) and (nil ~= _445_)) then
      local setfenv = _444_
      local loadstring = _445_
  ...
  local function doc_2a(tgt, name)
    if not tgt then
      ...
      if ((type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function"))) then
        ...
        return string.format("(%s%s%s)\n  %s", name, _448_, arglist, docstring)
      else
  ...
  local function __3estack(stack, tbl)
    for k, v in pairs(tbl) do
      local _457_ = stack
      table.insert(_457_, k)
      table.insert(_457_, v)
    end
    return stack
  end
  ...
  local function compile_value(v)
    ...
    local chunk = {}
    local _let_460_ = compiler.compile1(v, scope, chunk, opts)
    local _let_461_ = _let_460_[1]
    local v0 = _let_461_[1]
    return v0
  end
  ...
  local function get_fn_name(ast, scope, fn_name, multi)
    if (fn_name and (fn_name[1] ~= "nil")) then
      local _468_
      if not multi then
        _468_ = compiler["declare-local"](fn_name, {}, scope, ast)
      else
        _468_ = (compiler["symbol-to-expression"](fn_name, scope))[1]
      end
      return _468_, not multi, 3
    else
      return nil, true, 2
    end
  end
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    local _471_
    if local_3f then
      _471_ = "local function %s(%s)"
    else
      _471_ = "%s = function(%s)"
    end
    compiler.emit(parent, string.format(_471_, fn_name, table.concat(arg_name_list, ", ")), ast)
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    if (utils["string?"](expr) and (index_2a < #ast)) then
      local _475_
      do
        local _474_ = f_metadata
        _474_["fnl/docstring"] = expr
        _475_ = _474_
      end
      return _475_, index_2a
    elseif (utils["table?"](expr) and (index_2a < #ast) and assoc_table_3f(expr)) then
      local _476_
      do
        local tbl_11_auto = f_metadata
        for k, v in pairs(expr) do
          local _477_, _478_ = k, v
          if ((nil ~= _477_) and (nil ~= _478_)) then
            local k_12_auto = _477_
            local v_13_auto = _478_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
  ...
  SPECIALS.fn = function(ast, scope, parent)
    local f_scope
    do
      local _481_ = compiler["make-scope"](scope)
      do end (_481_)["vararg"] = false
      f_scope = _481_
    end
    local f_chunk = {}
  ...
  SPECIALS.lua = function(ast, _, parent)
    ...
    local _485_
    do
      local _484_ = utils["sym?"](ast[2])
      if (nil ~= _484_) then
        _485_ = tostring(_484_)
      else
        _485_ = _484_
      end
    end
    ...
    local _489_
    do
      local _488_ = utils["sym?"](ast[3])
      if (nil ~= _488_) then
        _489_ = tostring(_488_)
      else
        _489_ = _488_
      end
    end
  ...
  local function dot(ast, scope, parent)
    ...
    local len = #ast
    local _let_492_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local lhs = _let_492_[1]
    if (len == 2) then
      return tostring(lhs)
    else
      local indices = {}
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _let_493_ = compiler.compile1(index, scope, parent, {nval = 1})
          local index0 = _let_493_[1]
  ...
  local function disambiguate_3f(rootstr, parent)
    local function _502_()
      local _501_ = get_prev_line(parent)
      if (nil ~= _501_) then
        local prev_line = _501_
        return prev_line:match("%)$")
      else
        return nil
      end
    end
    return (rootstr:match("^{") or _502_())
  end
  SPECIALS.tset = function(ast, scope, parent)
    ...
    local keys = {}
    for i = 3, (#ast - 1) do
      local _let_504_ = compiler.compile1(ast[i], scope, parent, {nval = 1})
      local key = _let_504_[1]
  ...
  local function compile_until(condition, scope, chunk)
    if condition then
      local _let_513_ = compiler.compile1(condition, scope, chunk, {nval = 1})
      local condition_lua = _let_513_[1]
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    local _let_517_ = ast
    local _ = _let_517_[1]
    local _0 = _let_517_[2]
    local method_string = _let_517_[3]
    local call_string
  ...
  local function method_call(ast, scope, parent)
    ...
    local _let_519_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local target = _let_519_[1]
    local args = {}
    for i = 4, #ast do
      local subexprs
      local _520_
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local f_scope
    do
      local _525_ = compiler["make-scope"](scope)
      do end (_525_)["vararg"] = false
      _525_["hashfn"] = true
      f_scope = _525_
    end
    local f_chunk = {}
  ...
  local function maybe_short_circuit_protect(ast, i, name, _529_)
    local _arg_530_ = _529_
    local mac = _arg_530_["macros"]
  ...
  local function arithmetic_special(name, zero_arity, unary_prefix, ast, scope, parent)
    ...
    local _533_ = #operands
    if (_533_ == 0) then
      local _535_
      do
        local _534_ = zero_arity
        compiler.assert(_534_, "Expected more than 0 arguments", ast)
        _535_ = _534_
      end
      return utils.expr(_535_, "literal")
    elseif (_533_ == 1) then
  ...
  local function define_arithmetic_special(name, zero_arity, unary_prefix, _3flua_name)
    local _541_
    do
      local _538_ = (_3flua_name or name)
      local _539_ = zero_arity
      local _540_ = unary_prefix
      local function _542_(...)
        return arithmetic_special(_538_, _539_, _540_, ...)
      end
      _541_ = _542_
    end
    SPECIALS[name] = _541_
  ...
  local function define_bitop_special(name, zero_arity, unary_prefix, native)
    ...
    do
      local _549_ = native
      local _550_ = name
      local _551_ = zero_arity
      local _552_ = unary_prefix
      local function _554_(...)
        return bitop_special(_549_, _550_, _551_, _552_, ...)
      end
      _553_ = _554_
    end
    SPECIALS[name] = _553_
  ...
  local function native_comparator(op, _555_, scope, parent)
    local _arg_556_ = _555_
    local _ = _arg_556_[1]
    local lhs_ast = _arg_556_[2]
    local rhs_ast = _arg_556_[3]
    local _let_557_ = compiler.compile1(lhs_ast, scope, parent, {nval = 1})
    local lhs = _let_557_[1]
    local _let_558_ = compiler.compile1(rhs_ast, scope, parent, {nval = 1})
    local rhs = _let_558_[1]
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    local provided
    do
      local _566_ = (_3fopts or utils.root.options)
      if ((_G.type(_566_) == "table") and ((_566_)["compiler-env"] == "strict")) then
        provided = safe_compiler_env()
      elseif ((_G.type(_566_) == "table") and (nil ~= (_566_).compilerEnv)) then
        local compilerEnv = (_566_).compilerEnv
        provided = compilerEnv
      elseif ((_G.type(_566_) == "table") and (nil ~= (_566_)["compiler-env"])) then
        local compiler_env = (_566_)["compiler-env"]
  ...
  local _local_572_ = _573_(...)
  local dirsep = _local_572_[1]
  local pathsep = _local_572_[2]
  local pathmark = _local_572_[3]
  ...
  local function search_module(modulename, _3fpathstring)
    ...
    local function try_path(path)
      ...
      local _575_ = (io.open(filename) or io.open(filename2))
      if (nil ~= _575_) then
        local file = _575_
        file:close()
        return filename
    ...
    local function find_in_path(start, _3ftried_paths)
      local _577_ = fullpath:match(pattern, start)
      if (nil ~= _577_) then
        local path = _577_
        local _578_, _579_ = try_path(path)
        if (nil ~= _578_) then
          local filename = _578_
          return filename
        elseif ((_578_ == nil) and (nil ~= _579_)) then
          local error = _579_
          local function _581_()
            local _580_ = (_3ftried_paths or {})
            table.insert(_580_, error)
            return _580_
          end
          return find_in_path((start + #path + 1), _581_())
        else
          return nil
        end
      elseif true then
  ...
  local function make_searcher(_3foptions)
    local function _586_(module_name)
      local opts = utils.copy(utils.root.options)
      for k, v in pairs((_3foptions or {})) do
        opts[k] = v
      end
      opts["module-name"] = module_name
      local _587_, _588_ = search_module(module_name)
      if (nil ~= _587_) then
        local filename = _587_
        local _591_
        do
          local _589_ = filename
          local _590_ = opts
          local function _592_(...)
            return utils["fennel-module"].dofile(_589_, _590_, ...)
          end
          _591_ = _592_
        end
        return _591_, filename
      elseif ((_587_ == nil) and (nil ~= _588_)) then
        local error = _588_
        return error
      else
        return nil
  ...
  local function fennel_macro_searcher(module_name)
    local opts
    do
      local _594_ = utils.copy(utils.root.options)
      do end (_594_)["module-name"] = module_name
      _594_["env"] = "_COMPILER"
      _594_["requireAsInclude"] = false
      _594_["allowedGlobals"] = nil
      opts = _594_
    end
    local _595_ = search_module(module_name, utils["fennel-module"]["macro-path"])
    if (nil ~= _595_) then
      local filename = _595_
      local _596_
      if (opts["compiler-env"] == _G) then
        local _597_ = fennel_macro_searcher
        local _598_ = filename
        local _599_ = opts
        local function _601_(...)
          return dofile_with_searcher(_597_, _598_, _599_, ...)
        end
        _596_ = _601_
      else
        local _602_ = filename
        local _603_ = opts
        local function _605_(...)
          return utils["fennel-module"].dofile(_602_, _603_, ...)
        end
        _596_ = _605_
      end
      return _596_, filename
    else
      return nil
    end
  end
  local function lua_macro_searcher(module_name)
    local _608_ = search_module(module_name, package.path)
    if (nil ~= _608_) then
      local filename = _608_
      local code
      do
        ...
        code = close_handlers_8_auto(_G.xpcall(_610_, (package.loaded.fennel or debug).traceback))
      end
  ...
  local function search_macro_module(modname, n)
    local _612_ = macro_searchers[n]
    if (nil ~= _612_) then
      local f = _612_
      local _613_, _614_ = f(modname)
      if ((nil ~= _613_) and true) then
        local loader = _613_
        local _3ffilename = _614_
  ...
  local function _618_(modname)
    ...
    return (macro_loaded[modname] or sandbox_fennel_module(modname) or _619_())
  end
  safe_require = _618_
  ...
  local function resolve_module_name(_620_, _scope, _parent, opts)
    local _arg_621_ = _620_
    local filename = _arg_621_["filename"]
    local second = _arg_621_[2]
  ...
  local function include_path(ast, opts, path, mod, fennel_3f)
    ...
    do
      ...
      src = close_handlers_8_auto(_G.xpcall(_627_, (package.loaded.fennel or debug).traceback))
    end
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    local modexpr
    do
      local _630_, _631_ = pcall(resolve_module_name, ast, scope, parent, opts)
      if ((_630_ == true) and (nil ~= _631_)) then
        local modname = _631_
    ...
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      local function _635_()
        local _634_ = search_module(mod)
        if (nil ~= _634_) then
          local fennel_path = _634_
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _635_())
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"] or function(...)
  ...
  local function assert_compile(condition, msg, ast)
    if not condition then
      local _let_280_ = (utils.root.options or {})
      local source = _let_280_["source"]
      local unfriendly = _let_280_["unfriendly"]
      local error_pinpoint = _let_280_["error-pinpoint"]
  ...
  local function serialize_string(str)
    ...
    return string.gsub(string.gsub(string.format("%q", str), ".", serialize_subst), "[\128-\255]", _284_)
  end
  local function global_mangling(str)
    if utils["valid-lua-identifier?"](str) then
      ...
      return ("__fnl_global__" .. str:gsub("[^%w]", _285_))
    end
  end
  local function global_unmangling(identifier)
    local _287_ = string.match(identifier, "^__fnl_global__(.*)$")
    if (nil ~= _287_) then
      local rest = _287_
      local _288_
      local function _289_(_241)
        return string.char(tonumber(_241:sub(2), 16))
      end
      _288_ = string.gsub(rest, "_[%da-f][%da-f]", _289_)
      return _288_
    elseif true then
      local _ = _287_
      return identifier
  ...
  local function local_mangling(str, scope, ast, _3ftemp_manglings)
    ...
    local mangling
    local function _293_(_241)
      return string.format("_%02x", _241:byte())
    end
    mangling = string.gsub(string.gsub(raw, "-", "_"), "[^%w_]", _293_)
  ...
  local function autogensym(base, scope)
    local _296_ = utils["multi-sym?"](base)
    if (nil ~= _296_) then
      local parts = _296_
  ...
  local function flatten_chunk(file_sourcemap, chunk, tab, depth)
    if chunk.leaf then
      local _let_311_ = utils["ast-source"](chunk.ast)
      local filename = _let_311_["filename"]
      local line = _let_311_["line"]
  ...
  local function make_metadata()
    local function _320_(self, tgt, key)
      if self[tgt] then
        return self[tgt][key]
      else
        return nil
    ...
    local function _322_(self, tgt, key, value)
      self[tgt] = (self[tgt] or {})
      do end (self[tgt])[key] = value
      return tgt
    end
    local function _323_(self, tgt, ...)
      local kv_len = select("#", ...)
      local kvs = {...}
      if ((kv_len % 2) ~= 0) then
    ...
    return setmetatable({}, {__index = {get = _320_, set = _322_, setall = _323_}, __mode = "k"})
  end
  ...
  local function handle_compile_opts(exprs, parent, opts, ast)
    ...
    if opts.target then
      ...
      emit(parent, string.format("%s = %s", opts.target, _331_()), ast)
    else
    end
  ...
  local function find_macro(ast, scope)
    local macro_2a
    do
      local _335_ = utils["sym?"](ast[1])
      if (_335_ ~= nil) then
        local _336_ = tostring(_335_)
        if (_336_ ~= nil) then
          macro_2a = scope.macros[_336_]
        else
          macro_2a = _336_
        end
      else
  ...
  local function propagate_trace_info(_340_, _index, node)
    local _arg_341_ = _340_
    local filename = _arg_341_["filename"]
    local line = _arg_341_["line"]
    local bytestart = _arg_341_["bytestart"]
    local byteend = _arg_341_["byteend"]
    do
  ...
  local function macroexpand_2a(ast, scope, _3fonce)
    ...
    elseif (nil ~= _347_) then
      ...
      ok, transformed = xpcall(_349_, _350_())
      local _352_
      do
        local _351_ = ast
        local function _353_(...)
          return propagate_trace_info(_351_, ...)
        end
        _352_ = _353_
      end
      utils["walk-tree"](transformed, comp(_352_, quote_literal_nils))
  ...
  local function compile_function_call(ast, scope, parent, opts, compile1, len)
    ...
    for i = 2, len do
      ...
      subexprs = compile1(ast[i], scope, parent, {nval = _359_})
  ...
  local function compile_varg(ast, scope, parent, opts)
    ...
    if scope.hashfn then
      ...
      _364_ = "unexpected vararg"
    end
    assert_compile(scope.vararg, _364_, ast)
  ...
  local function serialize_number(n)
    local _367_ = string.gsub(tostring(n), ",", ".")
    return _367_
  end
  local function compile_scalar(ast, _scope, parent, opts)
    local serialize
    do
      local _368_ = type(ast)
      if (_368_ == "nil") then
        serialize = tostring
      elseif (_368_ == "boolean") then
        serialize = tostring
      elseif (_368_ == "string") then
        serialize = serialize_string
      elseif (_368_ == "number") then
  ...
  local function compile_table(ast, scope, parent, opts, compile1)
    local function escape_key(k)
      if ((type(k) == "string") and utils["valid-lua-identifier?"](k)) then
        ...
        local _let_370_ = compile1(k, scope, parent, {nval = 1})
        local compiled = _let_370_[1]
    ...
    do
      ...
      for i, elem in ipairs(ast) do
        ...
        do
          ...
          val_16_auto = exprs1(compile1(elem, scope, parent, {nval = nval}))
        end
        if (nil ~= val_16_auto) then
          i_15_auto = (i_15_auto + 1)
          do end (tbl_14_auto)[i_15_auto] = val_16_auto
    ...
    do
      local tbl_14_auto = buffer
      local i_15_auto = #tbl_14_auto
      for k, v in utils.stablepairs(ast) do
        local val_16_auto
        if not buffer[k] then
          local _let_373_ = compile1(ast[k], scope, parent, {nval = 1})
          local v0 = _let_373_[1]
          val_16_auto = string.format("%s = %s", escape_key(k), tostring(v0))
        else
          val_16_auto = nil
        end
        if (nil ~= val_16_auto) then
  ...
  local function destructure(to, from, ast, scope, parent, opts)
    local opts0 = (opts or {})
    local _let_377_ = opts0
    local isvar = _let_377_["isvar"]
    local declaration = _let_377_["declaration"]
    local forceglobal = _let_377_["forceglobal"]
    local forceset = _let_377_["forceset"]
    local symtype = _let_377_["symtype"]
    ...
    local function destructure_kv_rest(s, v, left, excluded_keys, destructure1)
      ...
      exclude_str = table.concat(_390_, ", ")
    ...
    local function destructure_table(left, rightexprs, top_3f, destructure1)
      ...
      do
        local _392_
        if top_3f then
          _392_ = exprs1(compile1(from, scope, parent))
        else
          _392_ = exprs1(rightexprs)
        end
        if (_392_ == "") then
          right = "nil"
        elseif (nil ~= _392_) then
          local right0 = _392_
          right = right0
        else
          right = nil
  ...
  local function traceback_frame(info)
    ...
    elseif (info.what == "C") then
      ...
      if (info.what == "Lua") then
        ...
        return string.format("  %s:%d: in function %s", info.short_src, info.currentline, _412_())
  ...
  local function traceback(_3fmsg, _3fstart)
    ...
    if ((msg:find("^%g+:%d+:%d+ Compile error:.*") or msg:find("^%g+:%d+:%d+ Parse error:.*")) and not utils["debug-on?"]("trace")) then
      ...
      while not done_3f do
        do
          local _416_ = debug.getinfo(level, "Sln")
          if (_416_ == nil) then
            done_3f = true
          elseif (nil ~= _416_) then
            local info = _416_
  ...
  local function do_quote(form, scope, parent, runtime_3f)
    ...
    elseif utils["list?"](form) then
      ...
      mapped = utils.kvmap(form, entry_transform(_424_, q))
      local filename
      if form.filename then
        filename = string.format("%q", form.filename)
      else
    ...
    elseif utils["sequence?"](form) then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s, sequence=%s})", mixed_concat(mapped, ", "), filename, _427_, "(getmetatable(sequence()))['sequence']")
    elseif (type(form) == "table") then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s})", mixed_concat(mapped, ", "), filename, _430_())
...
package.preload["fennel.friend"] = package.preload["fennel.friend"] or function(...)
    ...
    for pat, sug in pairs(suggestions) do
      ...
      if (0 < #matches) then
        ...
        for _, s0 in ipairs(sug) do
          local val_16_auto = s0:format(unpack(matches))
          if (nil ~= val_16_auto) then
            i_15_auto = (i_15_auto + 1)
            do end (tbl_14_auto)[i_15_auto] = val_16_auto
          else
  ...
  local function read_line(filename, line, _3fsource)
    if _3fsource then
      ...
      return close_handlers_8_auto(_G.xpcall(_196_, (package.loaded.fennel or debug).traceback))
    end
  end
  ...
  local function highlight_line(codeline, col, _3fendcol, opts)
    local _let_199_ = (opts or {})
    local error_pinpoint = _let_199_["error-pinpoint"]
    ...
    local _let_201_ = (error_pinpoint or {"\27[7m", "\27[0m"})
    local head = _let_201_[1]
    local tail = _let_201_[2]
    return (sub(codeline, 1, col) .. head .. sub(codeline, (col + 1), (endcol + 1)) .. tail .. sub(codeline, (endcol + 2), eol))
  end
  local function friendly_msg(msg, _202_, source, opts)
    local _arg_203_ = _202_
    local filename = _arg_203_["filename"]
    local line = _arg_203_["line"]
    local col = _arg_203_["col"]
    local endcol = _arg_203_["endcol"]
  ...
  local function assert_compile(condition, msg, ast, source, opts)
    if not condition then
      local _let_206_ = utils["ast-source"](ast)
      local filename = _let_206_["filename"]
      local line = _let_206_["line"]
      local col = _let_206_["col"]
...
package.preload["fennel.parser"] = package.preload["fennel.parser"] or function(...)
  ...
  local function granulate(getchunk)
    ...
    local function _208_(parser_state)
      if not done_3f then
        if (index <= #c) then
          ...
          local _209_ = getchunk(parser_state)
          local function _210_()
            local char = _209_
            return (char ~= "")
          end
          if ((nil ~= _209_) and _210_()) then
            local char = _209_
            c = char
            index = 2
            return c:byte()
  ...
  local function char_starter_3f(b)
    return ((function(_217_,_218_,_219_) return (_217_ < _218_) and (_218_ < _219_) end)(1,b,127) or (function(_220_,_221_,_222_) return (_220_ < _221_) and (_221_ < _222_) end)(192,b,247))
  end
  local function parser_fn(getbyte, filename, _223_)
    local _arg_224_ = _223_
    local source = _arg_224_["source"]
    local unfriendly = _arg_224_["unfriendly"]
    local comments = _arg_224_["comments"]
    local options = _arg_224_
    local stack = {}
    ...
    local function whitespace_3f(b)
      local function _234_()
        local t_233_ = options.whitespace
        if (nil ~= t_233_) then
          t_233_ = (t_233_)[b]
        else
        end
      ...
      return ((b == 32) or (function(_230_,_231_,_232_) return (_230_ <= _231_) and (_231_ <= _232_) end)(9,b,13) or _234_())
    end
    local function parse_error(msg, _3fcol_adjust)
      ...
      if (nil == utils["hook-opts"]("parse-error", options, msg, filename, (line or "?"), col0, source, utils.root.reset)) then
        ...
        if (unfriendly or not _G.io or not _G.io.read) then
          ...
          return friend["parse-error"](msg, filename, (line or "?"), col0, source, utils.root.options)
        end
      else
        return nil
      end
    ...
    local function parse_stream()
      ...
      local function dispatch(v)
        local _238_ = stack[#stack]
        if (_238_ == nil) then
          retval, done_3f, whitespace_since_dispatch = v, true, false
          return nil
        elseif ((_G.type(_238_) == "table") and (nil ~= (_238_).prefix)) then
          local prefix = (_238_).prefix
          local source0
          do
            local _239_ = table.remove(stack)
            set_source_fields(_239_)
            source0 = _239_
          end
      ...
      local function badend()
        ...
        return parse_error(string.format("expected closing delimiter%s %s", _241_, string.char(unpack(accum))))
      end
      ...
      local function parse_comment(b, contents)
        if (b and (10 ~= b)) then
          local function _245_()
            local _244_ = contents
            table.insert(_244_, string.char(b))
            return _244_
          end
      ...
      local function add_comment_at(comments0, index, node)
        local _248_ = (comments0)[index]
        if (nil ~= _248_) then
          local existing = _248_
      ...
      local function parse_string_loop(chars, b, state)
        ...
        do
          local _258_ = {state, b}
          if ((_G.type(_258_) == "table") and ((_258_)[1] == "base") and ((_258_)[2] == 92)) then
            state0 = "backslash"
          elseif ((_G.type(_258_) == "table") and ((_258_)[1] == "base") and ((_258_)[2] == 34)) then
            state0 = "done"
          elseif ((_G.type(_258_) == "table") and ((_258_)[1] == "backslash") and ((_258_)[2] == 10)) then
      ...
      local function parse_string()
        ...
        local _262_ = (rawget(_G, "loadstring") or load)(("return " .. formatted))
        if (nil ~= _262_) then
          local load_fn = _262_
      ...
      local function parse_number(rawstr)
        ...
        if rawstr:match("^%d") then
          ...
          local _268_ = tonumber(number_with_stripped_underscores)
          if (nil ~= _268_) then
            local x = _268_
            dispatch(x)
            return true
...
package.preload["fennel.view"] = package.preload["fennel.view"] or function(...)
  ...
  local function normalize_opts(options)
    ...
    for k, v in pairs(options) do
      ...
      local function _14_()
        local _13_ = v
        if ((_G.type(_13_) == "table") and (nil ~= (_13_).after)) then
          local val = (_13_).after
          return val
        else
          local function _15_()
            return v.once
          end
          if ((_G.type(_13_) == "table") and _15_()) then
      ...
      if ((nil ~= _11_) and (nil ~= _12_)) then
        local k_12_auto = _11_
        local v_13_auto = _12_
        tbl_11_auto[k_12_auto] = v_13_auto
      else
      end
    end
  ...
  local function sort_keys(_18_, _20_)
    local _arg_19_ = _18_
    local a = _arg_19_[1]
    local _arg_21_ = _20_
    local b = _arg_21_[1]
  ...
  local function concat_table_lines(elements, options, multiline_3f, indent, table_type, prefix, last_comment_3f)
    ...
    if (not getopt(options, "one-line?") and (multiline_3f or (options["line-length"] < (indent + length_2a(oneline))) or last_comment_3f)) then
      ...
      return (open .. table.concat(elements, indent_str) .. _45_() .. close)
    else
      return oneline
    end
  end
  ...
  local function pp_sequence(t, kv, options, indent)
    ...
    elseif (id and getopt(options, "detect-cycles?")) then
      ...
      do
        ...
        local tbl_14_auto = {}
        local i_15_auto = #tbl_14_auto
        for _, _56_ in ipairs(kv) do
          local _each_57_ = _56_
          local _0 = _each_57_[1]
          local v = _each_57_[2]
          local val_16_auto
          do
  ...
  local function pp_metamethod(t, metamethod, options, indent)
    if (options.depth <= options.level) then
      ...
      local _67_ = type(lines)
      if (_67_ == "string") then
        return lines
      elseif (_67_ == "table") then
  ...
  local function pp_table(x, options, indent)
    ...
    do
      ...
      if getopt(options, "metamethod?") then
        local _71_ = x
        if (nil ~= _71_) then
          local _72_ = getmetatable(_71_)
          if (nil ~= _72_) then
            _70_ = (_72_).__fennelview
          else
            _70_ = _72_
          end
        else
      ...
      if (nil ~= _70_) then
        local metamethod = _70_
        x0 = pp_metamethod(x, metamethod, options, indent)
      elseif true then
        local _ = _70_
        local _76_, _77_ = table_kv_pairs(x, options)
        if (true and (_77_ == "empty")) then
          local _0 = _76_
          if getopt(options, "empty-as-sequence?") then
            x0 = "[]"
          else
        ...
        elseif ((nil ~= _76_) and (_77_ == "table")) then
          local kv = _76_
          x0 = pp_associative(x, kv, options, indent)
        elseif ((nil ~= _76_) and (_77_ == "seq")) then
          local kv = _76_
  ...
  local function number__3estring(n)
    local _81_ = string.gsub(tostring(n), ",", ".")
    return _81_
  end
    ...
    local function validate_utf8(str0, index)
      ...
      local function _85_()
        ...
        for i = (index + 1), (index + init.len + -1) do
          ...
          code0 = (byte0 and code0 and (function(_87_,_88_,_89_) return (_87_ <= _88_) and (_88_ <= _89_) end)(128,byte0,191) and ((code0 * 64) + (byte0 - 128)))
        end
        return code0
      end
      code = (init and _85_())
      if (code and (function(_90_,_91_,_92_) return (_90_ <= _91_) and (_91_ <= _92_) end)(init["min-code"],code,init["max-code"]) and not (function(_93_,_94_,_95_) return (_93_ <= _94_) and (_94_ <= _95_) end)(55296,code,57343)) then
        return init.len
      else
        return nil
      end
  ...
  local function pp_string(str, options, indent)
    ...
    local function _101_(_241, _242)
  ...
  local function make_options(t, options)
    ...
    do
      ...
      for k, v in pairs(default_opts) do
        local _103_, _104_ = k, v
        if ((nil ~= _103_) and (nil ~= _104_)) then
          local k_12_auto = _103_
          local v_13_auto = _104_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
      defaults = tbl_11_auto
    end
  ...
  local function _106_(x, options, indent, colon_3f)
    ...
    local function _109_()
      local _108_ = getmetatable(x0)
      if (nil ~= _108_) then
        return (_108_).__fennelview
      else
        return _108_
      end
    end
    if ((tv == "table") or ((tv == "userdata") and _109_())) then
    ...
    elseif (tv == "number") then
      ...
      if ((tv == "string") and colon_string_3f(x0) and _111_()) then
        return (":" .. x0)
...
package.preload["fennel.utils"] = package.preload["fennel.utils"] or function(...)
  ...
  local len
  do
    local _116_, _117_ = pcall(require, "utf8")
    if ((_116_ == true) and (nil ~= _117_)) then
      local utf8 = _117_
      len = utf8.len
    elseif true then
      local _ = _116_
  ...
  local function stablepairs(t)
    ...
    if _122_ then
      ...
      local function _126_(_241, _242)
        return (tostring(_241) < tostring(_242))
      end
      table.sort(_124_, _126_)
      keys = _124_
    end
    local succ
    do
  ...
  local function member_3f(x, tbl, _3fn)
    local _149_ = tbl[(_3fn or 1)]
    if (_149_ == x) then
      return true
    elseif (_149_ == nil) then
      return nil
    elseif true then
      local _ = _149_
  ...
  local function allpairs(tbl)
    ...
    local function allpairs_next(_, state)
      ...
      elseif next_state then
        ...
        local _152_ = getmetatable(t)
        if ((_G.type(_152_) == "table") and true) then
          local __index = (_152_).__index
          if ("table" == type(__index)) then
            t = __index
  ...
  local expr_mt
  local function _158_(x)
    return tostring(deref(x))
  end
  expr_mt = {__tostring = _158_, "EXPR"}
  ...
  getenv = ((os and os.getenv) or _159_)
  ...
  local function sym(str, _3fsource)
    ...
    do
      ...
      for k, v in pairs((_3fsource or {})) do
        ...
        if ((nil ~= _161_) and (nil ~= _162_)) then
          local k_12_auto = _161_
          local v_13_auto = _162_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
    ...
    return setmetatable(_160_, symbol_mt)
  end
  nil_sym = sym("nil")
  local function sequence(...)
    local function _165_(seq, view0, inspector, indent)
      local opts
      do
        local _166_ = inspector
        _166_["empty-as-sequence?"] = {once = true, after = inspector["empty-as-sequence?"]}
        _166_["metamethod?"] = {once = false, after = inspector["metamethod?"]}
        opts = _166_
      end
      return view0(seq, opts, indent)
    end
    return setmetatable({...}, {sequence = sequence_marker, __fennelview = _165_})
  end
  local function expr(strcode, etype)
    return setmetatable({type = etype, strcode}, expr_mt)
  end
  local function comment_2a(contents, _3fsource)
    local _let_167_ = (_3fsource or {})
    local filename = _let_167_["filename"]
    local line = _let_167_["line"]
  ...
  local function varg(_3fsource)
    ...
    do
      ...
      for k, v in pairs((_3fsource or {})) do
        ...
        if ((nil ~= _169_) and (nil ~= _170_)) then
          local k_12_auto = _169_
          local v_13_auto = _170_
          tbl_11_auto[k_12_auto] = v_13_auto
        else
        end
      end
    ...
    return setmetatable(_168_, varg_mt)
  end
  ...
  root = {chunk = nil, scope = nil, options = nil, reset = _178_}
  root["set-reset"] = function(_179_)
    local _arg_180_ = _179_
    local chunk = _arg_180_["chunk"]
    local scope = _arg_180_["scope"]
    local options = _arg_180_["options"]
    local reset = _arg_180_["reset"]
    root.reset = function()
  ...
  local warned = {}
  local function check_plugin_version(_181_)
    local _arg_182_ = _181_
    local name = _arg_182_["name"]
    local versions = _arg_182_["versions"]
    local plugin = _arg_182_
  ...
  local function hook_opts(event, _3foptions, ...)
    ...
    local function _185_(...)
      local t_184_ = _3foptions
      if (nil ~= t_184_) then
        t_184_ = (t_184_).plugins
      else
      end
      return t_184_
    end
    local function _188_(...)
      local t_187_ = root.options
      if (nil ~= t_187_) then
        t_187_ = (t_187_).plugins
      else
      end
      return t_187_
    end
    plugins = (_185_(...) or _188_(...))
    if plugins then
      local result = nil
      for _, plugin in ipairs(plugins) do
        if result then break end
        check_plugin_version(plugin)
        local _190_ = plugin[event]
        if (nil ~= _190_) then
          local f = _190_
          result = f(...)
        else
...
local function eval(str, options, ...)
  ...
  loader = specials["load-code"](lua_source, env, _755_(...))
  opts.filename = nil
  return loader(...)
end
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _756_ = type(v)
    if (_756_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_756_ == "table") then
...
do
  ...
  package.preload[module_name] = _759_
  _ = nil
  local env
  do
    local _760_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_760_)["utils"] = utils
    _760_["fennel"] = mod
    env = _760_
  end
